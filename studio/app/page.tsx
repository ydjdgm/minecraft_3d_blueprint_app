...
export default function Home(){
 ...
 const [prompt,setPrompt]=useState(''),[apiKey,setApiKey]=useState(''),[model,setModel]=useState('gpt-5.4-mini'),[busy,setBusy]=useState(false);
 ...
 async function askAI(){if(!prompt.trim()||busy)return;if(!apiKey){setModal('settings');return;}setBusy(true);setProposal(null);const base=doc;try{const res=await fetch('/api/blueprint',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt,document:base,apiKey,model})});const data=await res.json() as Plan & {error?:string};if(!res.ok)throw Error(data.error||'AI 요청에 실패했습니다.');const result=applyPlan(base,data);if(live.current!==base){flash('설계도가 변경되었습니다. 최신 상태로 다시 요청하세요.');return;}setProposal({plan:data,result,base});}catch(e){flash((e as Error).message);}finally{setBusy(false);}}
 ...
 <aside className="details-panel">
  ...
  {tab==='materials'?<MaterialsPanel blocks={doc.blocks} progress={workspace.entry.progress} onProgress={workspace.progress}/>:<div className="ai-panel"><div className="ai-heading"><Sparkles size={24}/><h2>함께 설계해요</h2><p>새 건축물을 만들거나<br/>현재 설계도를 바꿔 보세요.</p></div>
   <button className="connection" onClick={()=>setModal('settings')}><Settings2 size={15}/>{apiKey?'모델 연결 설정':'API 키 연결하기'}</button>
 ...
 {modal&&<div className="modal-backdrop" onClick={e=>{if(e.target===e.currentTarget)setModal('');}}><section className={'modal '+(modal==='library'?'library-modal':'')} role="dialog" aria-modal="true" aria-label={modal==='settings'?'AI 연결 설정':'설계도 관리'}>
  ...
  {modal==='library'?<LibraryPanel library={workspace.library} onOpen={id=>{workspace.open(id);setModal('');}} onDuplicate={e=>workspace.add({...e.document,name:(e.document.name+' 복사본').slice(0,80)},e.progress)} onDelete={workspace.remove} onRestore={workspace.restore}/>:modal==='settings'?<><Settings2/><h2>AI 연결 설정</h2><p>OpenAI API 키로 설계를 생성합니다. 키는 현재 탭의 메모리에만 보관하며, 요청 처리 서버를 거쳐 OpenAI로 전달됩니다.</p><label>API 키<input type="password" value={apiKey} onChange={e=>setApiKey(e.target.value)} placeholder="sk-…" autoComplete="off"/></label><label>모델 ID<input value={model} onChange={e=>setModel(e.target.value)} maxLength={80}/></label><button className="primary" onClick={()=>{setModal('');flash(apiKey?'연결 설정을 저장했습니다. 설계를 요청해 보세요.':'API 키가 입력되지 않았습니다.');}}>설정 완료</button></>
